...
tsip_dialog.prototype.response_send = function(o_response) {
    ...
    if (o_layer_transac) {
        ...
        if (o_transac) {
            setTimeout(function() {
                o_transac.callback(tsip_transac_event_type_e.OUTGOING_MSG, o_response);
            }, 1);
            i_ret = 0;
        } else {
...
tsip_dialog.prototype.update_with_response = function(o_response) {
    ...
    if (i_code == 401 || i_code == 407 || i_code == 421 || i_code == 494) {
        ...
        /* Route sets */
        {
            ...
            for (i_index = 0;
                (o_hdr_recordRoute = o_response.get_header_at(tsip_header_type_e.Record_Route, i_index)); ++i_index) {
...
tsip_dialog.prototype.update_with_invite = function(o_invite) {
    ...
    /* Route sets */
    {
        ...
        this.ao_hdr_record_routes.splice(0, this.ao_hdr_record_routes.length);
        for (i_index = 0;
            (o_hdr_recordRoute = o_invite.get_header_at(tsip_header_type_e.Record_Route, i_index)); ++i_index) {
            this.ao_hdr_record_routes.push(o_hdr_recordRoute);
        }
    }
...
tsip_dialog.prototype.update_challenges = function(o_response, b_acceptNewVector) {
    ...
    */
    ...
    for (var i = 0;
        (o_hdr_WWW_Authenticate = o_response.get_header_at(tsip_header_type_e.WWW_Authenticate, i)); i++) {
        var b_isnew = true;
    ...
    for (var i = 0;
        (o_hdr_Proxy_Authenticate = o_response.get_header_at(tsip_header_type_e.Proxy_Authenticate, i)); i++) {
        var b_isnew = true;
...
tsip_dialog.prototype.get_newdelay = function(o_message) {
    ...
    for (i_index = 0; !b_found && (o_hdr = o_message.get_header_at(tsip_header_type_e.Contact, i_index)); ++i_index) {
        ...
        if (o_hdr_contact.o_uri) {
            ...
            if (o_contactUri) {
                if (tsk_string_equals(o_hdr_contact.o_uri.s_user_name, o_contactUri.s_user_name) && tsk_string_equals(o_hdr_contact.o_uri.s_host, o_contactUri.s_host) && o_hdr_contact.o_uri.i_port == o_contactUri.i_port) {
...
function tsip_dialog_compare(o_d1, o_d2) {
    if (o_d1 && o_d2) {
        if ((o_d1.s_callid == o_d2.s_callid) && ((o_d1.s_tag_local == o_d2.s_tag_local)) && ((o_d1.s_tag_remote == o_d2.s_tag_remote))) {
            return 0;
        }
    }
    return -1;
...
if (!window.__b_release_mode) {
    tsip_api_add_js_scripts('head',
        './webRTC/js' + version + '/src/tinySIP/src/dialogs/tsip_dialog_generic.js',
        './webRTC/js' + version + '/src/tinySIP/src/dialogs/tsip_dialog_invite.js',
        './webRTC/js' + version + '/src/tinySIP/src/dialogs/tsip_dialog_register.js'
    );
}